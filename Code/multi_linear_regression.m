...
f_test=length(fields(filtered_signals));
...
for f=1:length(fields(filtered_signals))
    
    fm = filtered_signals.filtered_signals_1.fm;  % Sample frequency.
    
    for stretch = 1:3   % Stretch
        %% DATA PREPARATION
        
        eval(sprintf('EEG = filtered_signals.filtered_signals_%d.EEG.stretch_%d;', f, stretch));  % EEG data.
        ...
        eval(sprintf('Z_prev.file_%d.stretch_angle_%d = filtered_signals.filtered_signals_%d.trajectories.stretch_%d(:,L*G-G+1:end)'';', f, stretch, f, stretch)); % Real trajectories matrix.

        eval(sprintf('Z_prev.file_%d.stretch_EEG_%d = EEG;', f, stretch)); % Real EEG
        eval(sprintf('Z_prev.file_%d.stretch_trajectory_%d = filtered_signals.filtered_signals_%d.trajectories.stretch_%d'';', f, stretch, f, stretch)); % Real trajectory
    end
end
